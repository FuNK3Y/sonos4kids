...
class Controller:
    ...
    async def connect(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"https://api.sonos.com/login/v3/oauth/access?grant_type=refresh_token&refresh_token={Config.sonos['refresh_token']}", headers=auth_headers) as response:
    ...
    async def load_favorite(self, favorite_id, action="REPLACE", play_on_completion=True):
        payload={"favoriteId": favorite_id, "action": action, "playOnCompletion": play_on_completion}
    ...
    async def modify_group_members(self, player_ids_to_add=[],player_ids_to_remove=[]):
        payload= {"playerIdsToAdd": player_ids_to_add,"playerIdsToRemove": player_ids_to_remove}
